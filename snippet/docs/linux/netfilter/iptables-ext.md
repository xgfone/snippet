...
在匹配扩展中，还有需要加 -m 引用模块的显示扩展，默认是隐含扩展，不要使用 -m
...
```
这样就把22与80合在一起了，OUTPUT同理，但要注意源端口目标端口，之后再删除原来的即可。
（注意：有SSH的话一定要先添加在删除）
...
```
表示所有客户端NEW请求进来、服务器返回ESTABLISHED的状态全部允许

也可以添加在INPUT中，提高查询速度
```
# iptables -I INPUT 1 -m state --state ESTABLISHED -j ACCEPT　
```
这里的原理是，**当NEW请求的时候匹配他自己的那条规则，当建立连接后，直接就匹配这条规则，提高效率**。

例如：第一条是允许ESTABISHED的，第二条是22的NEW、ESTABLISHED的规则，这样当用户请求22的时候先匹配22的NEW，之后的ESTABLISHED状态直接匹配第一条，不用在去匹配第二条的规则了，这样当我们有许多条目的时候大大提高了效率。
...
```
表示SSH记录中的IP在300s内发起超过3次连接则拒绝此IP的连接。
