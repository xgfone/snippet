...
**注：**在一个 crate 中，只有 Item 能被导出，其它的不能被导出。也就是说，在一个模块中，如果要访问其它模块，只能是是其它模块中的 Item，不能访问其它的东西。因此，一个模块中的变量，不能被其它模块访问，只能在当前模块内被使用。但是，一个模块中的变量可以通过一个函数的返回值来被其它模块使用。之所以这么设计，一个原因是涉及到变量的所有权的问题。
