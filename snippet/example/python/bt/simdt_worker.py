    ...
    def run(self):
        ...
        while True:
            ...
            if y:
                self.n_valid += 1
                # 更新最近发现时间，请求数
                sql = 'UPDATE search_hash SET last_seen=%s, requests=requests+1 WHERE info_hash=%s'
                self.dbcurr.execute(sql, (utcnow, info_hash))
            else:
                if dtype == 'pt':
            ...
            if self.n_reqs >= 1000:
                sql = ("INSERT INTO search_statusreport(date,new_hashes,total_requests, valid_requests) "
                       "VALUES(%s,%s,%s,%s) ON DUPLICATE KEY UPDATE total_requests=total_requests+%s, "
                       "valid_requests=valid_requests+%s, new_hashes=new_hashes+%s")
                self.dbcurr.execute(sql, (date, self.n_new, self.n_reqs, self.n_valid,
                                          self.n_reqs, self.n_valid, self.n_new))
                self.dbconn.commit()
